...
export class FinnishSSNControl implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  public updateView(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (newValue !== this._value) {
      ...
      if (!newValue) {
        ...
        const formatCheck = this._validateSSN(newValue);
  ...
  private _runValidationFlow(value: string, immediate: boolean = false): void {
    ...
    if (immediate) {
      ...
      this._debounceTimer = window.setTimeout(doCheck, 300);
    }
  }
  ...
  private _validateSSN(value: string): { valid: boolean; message?: string } {
    ...
    if (!match) return { valid: false, message: "Use format DDMMYY-123A" };
    ...
    const century = centuryMarker === "+" ? 1800 : centuryMarker === "-" ? 1900 : 2000;
  ...
  private _setStatus(state: StatusState): void {
    ...
    this._field.className = "ssn-container";
    switch (state) {
      case "empty":   this._status.innerHTML = this._svgEmpty(); break;
      case "valid":   this._field.className += " ssn-valid";   this._status.innerHTML = this._svgTick();  break;
      case "invalid": this._field.className += " ssn-invalid"; this._status.innerHTML = this._svgCross(); break;
      case "duplicate": this._field.className += " ssn-invalid"; this._status.innerHTML = this._svgCross(); break;
    }
  }
  ...
  private _svgEmpty(): string {
    return `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><circle cx="8" cy="8" r="6.5" stroke="#C8C6C4" /></svg>`;
  }
  private _svgTick(): string {
    return `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><circle cx="8" cy="8" r="7" fill="#DCFCE7" stroke="#86EFAC" stroke-width="1"/><polyline points="5,8.5 7,10.5 11,6" stroke="#16A34A" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>`;
  }
  private _svgCross(): string {
    return `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><circle cx="8" cy="8" r="7" fill="#FEE2E2" stroke="#FCA5A5" stroke-width="1"/><line x1="5.5" y1="5.5" x2="10.5" y2="10.5" stroke="#DC2626" stroke-width="1.5" stroke-linecap="round"/><line x1="10.5" y1="5.5" x2="5.5" y2="10.5" stroke="#DC2626" stroke-width="1.5" stroke-linecap="round"/></svg>`;
  }
  private _svgEye(): string {
    return `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M1.5 8C2.7 5.6 5.1 4 8 4s5.3 1.6 6.5 4c-1.2 2.4-3.6 4-6.5 4S2.7 10.4 1.5 8Z" stroke="#605E5C" stroke-width="1.2" /><circle cx="8" cy="8" r="2.2" stroke="#605E5C" stroke-width="1.2" /></svg>`;
  }
}
